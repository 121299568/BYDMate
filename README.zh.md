...
功能关闭时，应用不会改动任何系统设置。关闭开关会将设置恢复为出厂值。
...
为什么需要它：不同 BYD 车型的参数集不同。在 钛3 上开启座椅通风的命令，在宋上可能是另一个编号，甚至不存在。这正是部分功能在其他车型上失效的原因。你车辆的参数目录能显示它真实拥有哪些参数，这样我们就能依据你车辆的数据修复问题，而无需盲猜。
