...
已在 Leopard 3（方程豹 钛3）上测试，支持 Sea Lion 07、宋、Atto 3、Seal、汉。适配 DiLink 3.0、5.0、5.1 及搭载新版 UI7 界面的固件（OTA V1.6）。
...
**UI7 固件上的分屏。** 固件更新后，分屏由车辆自己完成：1/3 与 2/3 面板、分隔线和滑动交换手势都是原生的，BYDMate 只负责启动所选的两个应用。在这类固件上，设置里不会出现机制选择项，因为只有一种方式。

**"方向盘按键"触发器。** 任意方向盘按键都可以绑定自己的动作：自动化，添加触发器，"方向盘按键"，在学习模式下按下需要的按键。关闭或删除规则后，按键恢复出厂功能。
...
| **Voice** | 语音助手 | 俄语离线助手：车机本地语音识别、AI 助手（32 个工具）、神经网络语音合成 |
...
<img src="docs/screenshots/automation-en.jpg" alt="自动化" width="800">
...
<img src="docs/screenshots/split-screen.jpg" alt="分屏：左侧 Yandex Music，右侧 Yandex Navigator" width="800">
...
| | 描述 |
|---|-------------|
| **25 种触发器** | SOC、速度、温度、车门、车窗、天窗、胎压、驾驶模式、地理围栏、时段、精确时间和时间范围（含星期选择）、方向盘按键、点击小组件、语音指令、服务启动、网络连接恢复等 |
...
这是自动计算的结果。如果不合适，可在 **设置 → 车辆与电池 →"续航计算方式"** 中选择"手动"：自己填写 5 个点的表格，电池温度从 +20 到 -20 °C 及对应能耗，并可选填 100% 电量时的续航里程。
...
打开 **设置 → 显示**，开启"投射到仪表盘"。应用会自动启用所需的服务（需在安装时完成 ADB 激活，见"安装"章节），并自动将仪表盘切换到投射模式：由"立即显示到仪表盘"开关控制，默认开启。车机上无需手动设置。

之后，短按所选方向盘按键（默认右星键）即可将所选应用投射到仪表盘或切回中控屏。

如果关闭"立即显示到仪表盘"，需要自己把仪表盘切换到投射模式一次：点击车机主屏上的投射图标，然后在出现的地图上按 **IPC** 按钮。

<img src="docs/screenshots/settings-display-en.jpg" alt="设置 → 显示：仪表盘投屏各选项说明" width="800">
...
**设置 → 显示 → "仪表盘窗口大小"** 中共有五个滑块。如果你的车辆上投射画面本来就占满整个仪表盘（例如 Leopard 3），无需调整任何内容：默认值就是原来的全屏效果。

- **宽度**和**高度**（20%-100%）- 窗口占仪表盘的比例。缩小窗口可以腾出空间，周围会透出原生仪表盘内容。
- **水平偏移**（0%-100%）- 窗口位置：0 为左边缘，50 为居中，100 为右边缘。仅在宽度小于 100% 时生效。
- **垂直偏移**（0%-100%）- 垂直方向同理：0 为顶部，50 为居中，100 为底部。
- **缩放**（50%-150%）- 投射应用界面的字体和元素大小：数值越小，文字越小，地图显示的范围越大。

调整会立即生效，投射过程中拖动滑块即可在仪表盘上看到效果。

### 带仪表盘迷你窗口的车型（Sea Lion 07 等）

部分车型的原厂仪表盘并非全屏显示投射画面，而是显示在一个固定区域内（Sea Lion 07 上是右侧三分之一）。全屏窗口在这些车型上会被裁切。解决方法：用上面的滑块缩小窗口并将其移动到可见区域。

Sea Lion 07 车主验证过的数值：**宽度 32%，高度 92%，水平偏移 100%，垂直偏移 6%，缩放 90%**。

<img src="docs/screenshots/cluster-mini-settings.jpg" alt="Sea Lion 07 的窗口设置" width="800">

<img src="docs/screenshots/cluster-mini-view.jpg" alt="Yandex 导航在 Sea Lion 07 仪表盘迷你区域中显示" width="800">

在这类车型上无法让窗口占满整个仪表盘：显示区域由仪表盘固件决定，无法控制。
...
<img src="docs/screenshots/settings-split-en.jpg" alt="设置 → 分屏" width="800">
...
在搭载新版 UI7 界面的固件上不会出现机制选择项：分屏由车辆自己完成，1/3 与 2/3 面板是原生的，开启时无需重启车机。
...
- **电池容量**：默认为 72.9 kWh（ 钛3）。前往 **设置 → 车辆与电池** 设置你自己的容量。例如：Atto 3 = 60.5 kWh，Seal AWD = 82.5 kWh，汉 EV = 85.4 kWh。否则续航和行程费用计算将不准确。
...
如果有功能不工作或显示异常数据，请提交 [Issue](https://github.com/AndyShaman/BYDMate/issues)，附上你的车型和 DiLink 固件版本，并附上你车辆的参数目录：**设置 → 服务与数据 → "保存 fid 目录"**。文件 `fid-dump-….txt` 会出现在 Download 文件夹中。文件里只有技术参数标识符，不含 VIN、不含位置信息、不含任何个人数据。
...
| 遥测 Webhook | 设置 → 集成 → Webhook | 与 ABRP 相同的 JSON，POST 到你的 URL，每 1/8/30 秒发送一次（行驶/充电/停车）；GPS 坐标仅在单独开关开启时发送 | 默认关闭 |
...
没有 ADB 时，BYDMate 作为离线行程日志运行：来自原车数据库的历史记录、路线地图、CSV 导出、基于已有数据的 AI 洞察。

开启 ADB 调试是读取或写入车辆参数的一切功能所必需的：悬浮窗和仪表盘上的实时数值（SOC、续航、温度、12V）、SoH、自动充电日志、自动化、投射到仪表盘、分屏、HUD 提示、盲区摄像头、语音控制车辆以及日志记录。
...
1. 打开 BYDMate — 出现设置向导
2. 授予**位置**和**存储**权限。随着功能陆续开启，应用还会请求：语音助手所需的麦克风、盲区摄像头所需的相机、按姓名拨打电话所需的通讯录、悬浮窗和通知所需的悬浮窗权限，以及用于识别当前活动应用的使用情况访问权限
3. 设置**电价**（用于行程费用计算）
...
**设置 → 服务与数据 → 车辆系统** 中有两个选项，改变的是车辆本身的行为，而不是应用的行为。

**方向盘音量旋钮。** 开启此开关后，按下旋钮会暂停/继续播放，而不是切换音源。适用于固件更新后按下旋钮变成切换音源的情况。默认关闭。

**车机界面语言。** 仅适用于 UI7 固件。此按钮会打开 BYD 隐藏的原厂语言对话框，包含完整语言列表，含俄语、乌克兰语和波兰语。语言由固件本身更改，应用不写入任何内容。部分车主切换为俄语后自动泊车会失效，此时用同一按钮切回 English 即可。
...
<img src="docs/screenshots/settings-integrations-en.jpg" alt="设置 → 集成：ABRP" width="800">
